...
- **Smoke runs should include visible learning artifacts.** The Qwen3.6 overlay now runs 64 steps over 1024 synthetic samples by default, writes `metrics/loss.csv` plus `metrics/loss.svg`, and runs three held-out prompts against both the base and fine-tuned model. The prompt comparison is written to `eval/sample-compare.md` under the EFS run directory.
