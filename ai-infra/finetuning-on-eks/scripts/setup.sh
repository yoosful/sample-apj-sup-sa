...
configure_training() {
    ...
    case "$model" in
        ...
        qwen3.6-35b-a3b-megatron)
            ...
            echo "  Dataset:   Qwen/SWIFT official text instruction mix"
            ;;
    esac
...
run_megatron_swift_training() {
    ...
    echo "  kubectl logs -n ml-training job/qwen-ep-benchmark | grep 'BENCH_SAMPLE_'"
    echo "  Loss CSV/SVG are written under /data/qwen-ep-bench/<run-id>/metrics on the EFS PVC."
    echo "  Before/after prompt comparisons are written under /data/qwen-ep-bench/<run-id>/eval."
    echo ""
    echo "Stop:"
    echo "  ./setup.sh stop"
}
